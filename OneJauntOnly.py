...
# @profile
def jaunt(current_pos, channel_dict, channels):
    jaunt_to_ = current_pos
    jaunt_to = current_pos
    poss_jaunts = channel_dict[current_pos[0]]
    for j in poss_jaunts:
        if current_pos[1] == j[1] and current_pos[2] == j[2]:
            jaunt_to_ = list(j)
    for i in channels:
        if current_pos == [i[0], i[1], i[2]]:
            ...
            # break
    if tuple(jaunt_to_) != tuple(jaunt_to):
        print('jaunt_old = {}'.format(jaunt_to))
        print('jaunt_new = {}\n'.format(jaunt_to))
        # exit()
    return jaunt_to
...
# @profile
def next_position(world_grid, current_pos, direction, channel_dict, channels):
    ...
    if direction == 'North':
        ...
        next_point = jaunt(current_pos, channel_dict, channels)
...
# @profile
def breadth_first(world_grid, start_state, end_state, channel_dict, channels, node_channel):
    cost = 0
    frontier = deque([])
    ...
    while True:
        ...
        actions_available(world_grid, curr_node[0], node_channel)
        for action in actions:
            child = [next_position(world_grid, curr_node[0], action, channel_dict, channels),
                     curr_node[1] + 1]
...
def uniform_cost(world_grid, start_state, end_state, channel_dict, channels, node_channel):
    cost = 0
    counter = 0
    ...
    while True:
        ...
        for action in actions:
            next_node = next_position(world_grid, curr_node[2], action, channel_dict, channels)
...
# @profile
def a_star(world_grid, start_state, end_state, channel_dict, channels, node_channel):
...
st = time.time()
if algorithm == "BFS":
    breadth_first(world, start, end, channel_dict_global, ch, channel_dict_node)
elif algorithm == "UCS":
    uniform_cost(world, start, end, channel_dict_global, ch, channel_dict_node)
elif algorithm == "A*":
    a_star(world, start, end, channel_dict_global, ch, channel_dict_node)
